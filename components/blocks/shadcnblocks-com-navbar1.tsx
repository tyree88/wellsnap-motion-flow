...
const Navbar1 = ({
  ...
  const handleNavigation = (url: string, e?: React.MouseEvent) => {
    ...
    if (url.startsWith("#")) {
      const didScroll = scrollToSection(url)
      if (!didScroll && url === "#") {
        router.push("/")
      }
    } else {
      ...
      <div className="container">
        <nav className="hidden justify-between lg:flex">
          <div className="flex items-center gap-6">
            <a href={logo.url} className="flex items-center gap-2" onClick={(e) => handleNavigation(logo.url, e)}>
              <img src={logo.src || "/placeholder.svg"} className="w-8" alt={logo.alt} />
              <span className="text-lg font-semibold">{logo.title}</span>
            </a>
            <div className="flex items-center">
              <NavigationMenu>
                <NavigationMenuList>{menu.map((item) => renderMenuItem(item, handleNavigation))}</NavigationMenuList>
              </NavigationMenu>
            </div>
          </div>
          <div className="flex gap-2">
            <Button asChild variant="outline" size="sm">
              <a href={auth.login.url} onClick={(e) => handleNavigation(auth.login.url, e)}>
                {auth.login.text}
              </a>
            </Button>
            <Button asChild size="sm">
              <a href={auth.signup.url} onClick={(e) => handleNavigation(auth.signup.url, e)}>
                {auth.signup.text}
              </a>
            </Button>
          </div>
                ...
                <SheetHeader>
                  <SheetTitle>
                    <a
                      ...
                      onClick={(e) => handleNavigation(logo.url, e)}
                    >
                      <img src={logo.src || "/placeholder.svg"} className="w-8" alt={logo.alt} />
                      <span className="text-lg font-semibold">{logo.title}</span>
                    </a>
...
const renderMobileMenuItem = (item: MenuItem, handleNavigation: (url: string, e?: React.MouseEvent) => void) => {
  ...
  return (
    <a key={item.title} href={item.url} className="font-semibold" onClick={(e) => handleNavigation(item.url, e)}>
      {item.title}
    </a>
  )
}
